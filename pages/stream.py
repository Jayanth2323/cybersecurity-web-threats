...
def load_data():
    try:
        ...
        df = df.dropna(subset=["anomaly", "bytes_in", "bytes_out", "duration_seconds", "avg_packet_size"])
        le = LabelEncoder()
...
# SHAP Analysis - Corrected implementation
explainer = shap.TreeExplainer(model)
shap_values = explainer.shap_values(X_test)

# For binary classification, we get [negative_class, positive_class] arrays
shap_values_pos = np.array(shap_values[1])  # Using positive class values
base_value = float(explainer.expected_value[1])  # Convert to float

# Verify dimensions
st.write(f"SHAP values shape: {shap_values_pos.shape}")
st.write(f"X_test shape: {X_test.shape}")

# Ensure dimensions match
if shap_values_pos.shape != X_test.shape:
    st.error("Dimension mismatch between SHAP values and test data!")
    st.stop()
...
st.markdown(f"**Prediction:** {'Suspicious' if sample_pred == 1 else 'Normal'}")
...
with st.expander("📊 Top Contributing Features"):
    try:
        ...
        sample_shap = shap_values_pos[selected_index]
